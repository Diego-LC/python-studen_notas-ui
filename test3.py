...
class VentanaNotas:
    def __init__(self, ventana):
        ...
        self.seleccion = ""
        self.datos = [{'ponderaciones':{'tipo_evaluacion':{'Eval. Práctica':0.4, 'Eval. Teórica':0.6}, 
        ...
        btn_calcular_promedios = tk.Button(ventana, text="Calcular Promedios", command=self.calcular_promedios)
        btn_calcular_promedios.pack(pady=5)

        seleccion = ttk.Combobox(ventana, values=["Promedio Final", "Promedio Parcial"])
        seleccion.pack(pady=5)

        btn_opciones = tk.Button(ventana, text="Opciones", command=self.mostrar_opciones)
        btn_opciones.pack(pady=5)
    ...
    def editar_fila(self, event):
        ...
        combobox = ttk.Combobox(ventana_editar, textvariable=tk.StringVar(), values=tipo_evaluacion)
        combobox.current(tipo_evaluacion.index(valores[3]))
    ...
    def calcular_promedios(self):
        ...
        promedios_nota = {}
        promedio_total = 0.0
# Falta truncar los promedios a máximo 2 decimales
